...
export class EventService{
  ...
  updateEvent(event: any){
    let index=EVENT.findIndex(x => x.id=event.id)
    EVENT[index]=event
  }
  searchSessions(searchTerms: string){
    var term=searchTerms.toLocaleLowerCase();
    var results:ISession[]=[]

    EVENT.forEach(element => {
      var match=element.sessions.filter(session=>session.name.toLocaleLowerCase().indexOf(term)>-1);
      match=match.map((session:any) => {
        session.eventId=element.id;
        return session
      })
     results=results.concat(match)
    });
    var emit = new EventEmitter(true);
    setTimeout(()=>{
      emit.emit(results)
    },500);
    return emit
